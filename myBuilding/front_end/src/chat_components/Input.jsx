...
export const Input = () => {
  ...
  const handleSend = async () => {
    if (img) {
      ...
      try {
        //for updating the actual "chats collection"
        await updateDoc(doc(db, "chats", data.chatId), {
          messages: arrayUnion({
    ...
    <div className=" p-2.5  font-mono">
      <div className=" flex flex-row space-x-1 justify-around white cursor-pointer">
        <input
          ...
          value={text}
        />
        <div className="w-2/6 flex flex-row items-center justify-between space-x-2">
          <img
          ...
          <button
            onClick={handleSend}
            className="w-14 h-10 tracking-wider text-sm bg-green-700 rounded-sm hover:bg-green-600"
          >
            send
          </button>
        </div>
